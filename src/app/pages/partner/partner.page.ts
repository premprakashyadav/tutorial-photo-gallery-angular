...
if (this.photoService.photos.length > 0) {
    ...
    dataPost.append('orgName', this.orgName);
    dataPost.append('personName', this.personName);
    dataPost.append('personContact', this.personContact);
    dataPost.append('inputPatientEmail', this.email);
    dataPost.append('message', this.message);
    if(this.prescription.length > 0 || this.message || this.orgName || this.personName || this.personContact) {
    this.providerSvc.postData("appointment_partner.php", dataPost).subscribe(async (res: any) => {
      if(res) {
